...
server <- function(input, output, session) {
  ...
  output$data_table <- renderDT({
    ...
    if (input$filter_partner != "") {
      dt.filtered.data <-
        dt.filtered.data[dt.filtered.data$partner_name %in% input$filter_partner, ]
    }
    if (input$filter_reporter != "") {
      dt.filtered.data <-
        dt.filtered.data[dt.filtered.data$reporter_name %in% input$filter_reporter, ]
    }
    if (input$filter_year != "") {
      dt.filtered.data <- dt.filtered.data[dt.filtered.data$year == input$filter_year, ]
    }
  ...
  df.filter.data <- reactive({
    l.country <- sort(input$comp_countryInput)
    
    dt.merged[reporter_name == l.country[1] & year >= input$year[1] & year <= input$year[2]]
  })
  ...
  df.filter.data.2 <- reactive({
    ...
    if (!is.null(l.country[2])) {
      dt.merged[reporter_name == l.country[2] & year >= input$year[1] & year <= input$year[2]]
    } else {
      NULL
    }
  })
  ...
  df.filter.data.3 <- reactive({
    ...
    if (!is.null(l.country[3])) {
      dt.merged[reporter_name == l.country[3] & year >= input$year[1] & year <= input$year[2]]
    } else {
      NULL
    }
  })
  ...
  output$plot <- renderPlot({
    ...
    if (!is.null(df.filter.data.2()) & !is.null(df.filter.data.3())) {
      ggplot() +
        ...
        geom_line(data = df.filter.data.3(), aes_string(x = "year", y = input$column, color = "'Country 3'")) +
        labs(x = "Year",
  ...
  output$map <- renderLeaflet({
    ...
    df.selected.countries <- unique(dt.merged[reporter_name %in% input$comp_countryInput,
