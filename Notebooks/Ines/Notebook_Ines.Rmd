...
g.trade <- graph_from_edgelist(m.trade, directed=TRUE)
  # Eliminate self-loops (first we must remove the loops out of trade_value_usd before we can do this)
  #g.trade <- simplify(g.trade, remove.multiple = FALSE, remove.loops = TRUE) #eliminate self-loops
...
# Reorder the attributes to match the order that the individuals appear as network vertices
countries <- dt.trade.year[reporter_name %in% V(g.trade)$name,]
countries <- countries[match(V(g.trade)$name, countries$reporter_name), ]
# We assign the dataframe columns as vertex attributes, e.g., 
V(g.trade)$continent <- countries$reporter_continent
