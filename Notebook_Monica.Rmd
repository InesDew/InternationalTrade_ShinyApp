...
# Get the edge and vertex coordinates
edges_df <- data.frame(get.edgelist(g1))


vertices_df <- get.data.frame(g1, what = "vertices")
# Get the vertex attributes
#vertices_df <- as_data_frame(V(g1))
vertices_df$long <- ifelse(V(g1)$type, vertices_df$reporter_long, vertices_df$partner_long)
vertices_df$lat <- ifelse(V(g1)$type, vertices_df$reporter_lat, vertices_df$partner_lat)
